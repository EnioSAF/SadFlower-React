    ...
    <div className="window-body mywork-body">
      ...
      <div className="browser-page">
        ...
        {tab !== 'Portfolio' && <>
          ...
          {tab === 'Quick quote' && <section className="quote-layout"><div className="retro-panel selected-service"><small>SELECTED LISTING</small><h2>{selectedService.label}</h2><p>{selectedService.description}</p><button onClick={() => setTab('Services')}>Change service</button></div><form className="quote-form" onSubmit={submit}><h2>Quick quote / reservation</h2><label>Name *<input value={form.name} onChange={(event) => update('name', event.target.value)} /></label><label>Email *<input type="email" value={form.email} onChange={(event) => update('email', event.target.value)} /></label><label>Phone<input value={form.phone} onChange={(event) => update('phone', event.target.value)} /></label><label>Preferred date<input type="date" value={form.preferredDate} onChange={(event) => update('preferredDate', event.target.value)} /></label><label>Message<textarea rows="4" value={form.message} onChange={(event) => update('message', event.target.value)} /></label><label className="consent"><input type="checkbox" checked={form.consent} onChange={(event) => update('consent', event.target.checked)} /> I agree to be contacted about this request.</label><button type="submit" disabled={state === 'loading'}>{state === 'loading' ? 'Sending...' : 'SEND REQUEST >>'}</button>{state === 'invalid' && <p className="form-error">Name, valid email and consent are required.</p>}{state === 'success' && <p className="form-success">Request received: {result?.requestId}</p>}{state === 'error' && <p className="form-error">{result}. Please try again later.</p>}</form></section>}
        </>}
      </div>
